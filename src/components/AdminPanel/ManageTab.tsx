...
export default function ManageTab({ themes, setThemes, saveThemes, systems, categories }: ManageTabProps) {
  ...
  const handleExport = () => {
    const data = themes.map(theme => ({
      ...
      gameId: theme.gameId, mature: theme.mature
    }));
  ...
  const handleGithubPush = async (token: string) => {
    ...
    try {
      ...
      const allThemesData = themes.map(theme => ({
        ...
        downloadUrl: reverseConvertUrl(theme.downloadUrl), size: theme.size,
        date: theme.date, onScreenScraper: theme.onScreenScraper, isMulti: theme.isMulti,
        ssGameId: theme.ssGameId, gameId: theme.gameId, mature: theme.mature
      }));
      ...
      <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl border-2 border-orange-500 max-w-3xl w-full shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={e => { e.preventDefault(); onSave(editData); }} className="p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <label className="flex items-center gap-3 p-4 bg-gray-950 border border-gray-700 rounded-xl cursor-pointer hover:border-red-500 transition">
                <input type="checkbox" checked={editData.mature || false}
                  onChange={e => setEditData({...editData, mature: e.target.checked})}
                  className="w-5 h-5 rounded border-gray-600 text-red-600 focus:ring-2 focus:ring-red-500/20 focus:ring-offset-0 bg-gray-800 cursor-pointer" />
                <div className="flex items-center gap-2">
                  <svg className="w-5 h-5 text-red-400" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                  </svg>
                  <span className="text-sm font-bold text-gray-300">Contenu mature (visuel flouté sur la vitrine, +18 pour révéler)</span>
                </div>
              </label>
            </div>
            <div className="md:col-span-2">
